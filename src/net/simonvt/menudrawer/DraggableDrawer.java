...
//					postDelayed(mPeekStartRunnable, 1000);
	...
	protected boolean canChildrenScroll(int dx, int dy, int x, int y) {
		...
		case RIGHT:
			if (!mMenuVisible) {
				canScroll = canChildScrollHorizontally(mContentContainer,
						false, dx, x - ViewHelper.getLeft(mContentContainer), y
								- ViewHelper.getTop(mContentContainer));
			} else {
				canScroll = canChildScrollHorizontally(mMenuContainer, false,
						dx, x - ViewHelper.getLeft(mMenuContainer), y
								- ViewHelper.getTop(mContentContainer));
			}
			break;
		...
		case BOTTOM:
			if (!mMenuVisible) {
				canScroll = canChildScrollVertically(mContentContainer, false,
						dy, x - ViewHelper.getLeft(mContentContainer), y
								- ViewHelper.getTop(mContentContainer));
			} else {
	 ...
	 */
	protected boolean canChildScrollHorizontally(View v, boolean checkV,
			int dx, int x, int y) {
		if (v instanceof ViewGroup) {
			...
			for (int i = count - 1; i >= 0; i--) {
				...
				final int childLeft = child.getLeft()
						+ supportGetTranslationX(child);
				final int childRight = child.getRight()
						+ supportGetTranslationX(child);
				final int childTop = child.getTop()
						+ supportGetTranslationY(child);
				...
				if (x >= childLeft
						...
						&& y >= childTop
						&& y < childBottom
						&& canChildScrollHorizontally(child, true, dx, x
								- childLeft, y - childTop)) {
					return true;
				}
			}
		}
		...
		return checkV
				&& mOnInterceptMoveEventListener.isViewDraggable(v, dx, x, y);
	}
	 ...
	 * @return true if child views of v can be scrolled by delta of dx.
	 */
	protected boolean canChildScrollVertically(View v, boolean checkV, int dx,
			int x, int y) {
		if (v instanceof ViewGroup) {
			final ViewGroup group = (ViewGroup) v;
			...
			for (int i = count - 1; i >= 0; i--) {
				...
				if (x >= childLeft
						...
						&& y >= childTop
						&& y < childBottom
						&& canChildScrollVertically(child, true, dx, x
								- childLeft, y - childTop)) {
					return true;
				}
			}
